...
/// Prefers qty × unit cost (including master roll-through) over flat line_cost.
...
mod tests {
    ...
    #[test]
    fn unit_cost_beats_line_cost() {
        let mut ing = sample_ing("2", "1.00");
        ing.line_cost = Some(Decimal::from(99));
        let cost = ingredient_line_cost(&ing, Decimal::ONE).unwrap();
        assert_eq!(cost, Decimal::from(2));
    }
}
