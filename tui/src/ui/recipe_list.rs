...
pub fn render(frame: &mut Frame, area: Rect, state: &mut RecipeListState, status: &str) {
        ...
        state
            ...
            .map(|r| {
                ...
                ListItem::new(Line::from(vec![
                    Span::styled(
                        format!("[{}] ", difficulty),
                        Style::default().fg(T.difficulty(r.difficulty)),
                    ),
                    Span::raw(&r.name),
                    Span::styled(format!(" ({}) ", time), Style::default().fg(T.muted)),
                    Span::styled(rating, Style::default().fg(T.medium)),
                ]))
            })
            .collect()
    };
    ...
    let footer = Paragraph::new(footer_text).style(Style::default().fg(T.muted));
