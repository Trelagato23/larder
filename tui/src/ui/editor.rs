...
impl EditorState {
    pub fn new(
        ...
        Self {
            ...
            menu_price: recipe
                ...
                .map(|p| p.to_string())
                .unwrap_or_default(),
            allergens: recipe.allergens.clone().unwrap_or_default(),
            recipe,
    ...
    pub fn build_recipe(&self) -> Result<Recipe, String> {
        ...
        Ok(Recipe {
            ...
            menu_price,
            yield_quantity: self.recipe.yield_quantity,
            yield_unit: self.recipe.yield_unit.clone(),
            waste_percent: self.recipe.waste_percent,
            author: self.recipe.author.clone(),
            estimated_calories: self.recipe.estimated_calories,
            allergens: larder_core::models::normalize_allergens(Some(self.allergens.as_str())),
            last_opened_at: None,
            open_count: None,
        })
    }
    ...
    pub fn next_field(&mut self) {
        self.field = match self.field {
            ...
            EditField::MenuPrice => EditField::Allergens,
            EditField::Allergens => EditField::Name,
        };
    }

    pub fn prev_field(&mut self) {
        self.field = match self.field {
            EditField::Name => EditField::Allergens,
            ...
            EditField::Allergens => EditField::MenuPrice,
        };
    }
    ...
    pub fn push_char(&mut self, c: char) {
        ...
        match self.field {
            ...
            EditField::Allergens => self.allergens.push(c),
        }
    }
...
fn render_meta(frame: &mut Frame, area: Rect, state: &EditorState) {
    ...
    let lines = vec![
        ...
        Line::from(vec![
            Span::styled("Allergens: ", field_style(EditField::Allergens)),
            Span::styled(
                format!(
                    "{}{}",
                    state.allergens,
                    if state.field == EditField::Allergens {
                        cursor
                    } else {
                        ""
                    }
                ),
                field_style(EditField::Allergens),
            ),
        ]),
